...
class Watchlist extends House {
    ...
    public function houseWatch_ListHome($categories,$pages,$user_id)
    {
        ...
        WHERE W. categories ='$categories' AND W. user_id3_list= '$user_id' ORDER BY modified ,rand() Desc Limit $showpages,10");
        ?>
                            ...
                            <div class="text-muted clear-right" style="padding-bottom: 10px;">
                                <form method="post" id="form-housecartitem<?php echo $house['code']; ?>remove"  class="float-right" >
                                       <input type="hidden" style="width:30px;" name="user_id" value="<?php echo $house['user_id3_list']; ?>" />
                                        ...
                                        <button type="button"  onclick="xxda_watch_list_delete('remove','<?php echo 'form-housecartitem'.$house['code'].'remove'; ?>','<?php echo $house['code']; ?>');"  class="btn btn-outline-danger btn-sm " ><img src="<?php echo BASE_URL_LINK ;?>image/img/icon-delete.png" alt="Remove Item" /> Remove</button> 
                                </form>
                        </div>
                </div>
